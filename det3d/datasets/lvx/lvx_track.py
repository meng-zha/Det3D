...
def lvx_track(annos):
    ...
    for i,anno in enumerate(annos):
        if i == 0:
            ...
            get_track(past_predict,-0.25,track_ids,scores, annos[i-1],flag)
            
            predict_pre = get_track_scores(boxes,boxes_1)
            get_track(predict_pre,-0.25,track_ids,scores,annos[i-1],flag)
            ...
            get_track(virtual,-0.75,track_ids,scores,vir_annos,flag_vir)
            ...
            # 向下一帧移动   
            vir_annos['location_1'] = vir_annos['location']
            vir_annos['location'] = vir_annos['location_2']
            flag_vir = []
            for j in range((len(vir_annos['track_id']))):
                vir_annos['location_2'][j] = 2*vir_annos['location'][j]-vir_annos['location_1'][j]
...
def get_track(iou, threshold, track, scores, anno, flag):
    num = 0
    while(num<iou.shape[0] and num<iou.shape[1]):
        loc_max = iou.argmax()
        max_iou = [loc_max//iou.shape[1],loc_max%iou.shape[1]]
        score = iou[max_iou[0],max_iou[1]]
        if score <threshold:
            break
        if track[max_iou[0]] == -1 and max_iou[1] not in flag:
