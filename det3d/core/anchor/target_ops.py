...
def create_target_np_track(
    ...
    if positive_fraction is not None:
        ...
        if len(gt_boxes) == 0:
            ...
            labels[anchors_with_max_overlap] = gt_classes[gt_inds_force]
    bbox_targets = np.zeros((num_inside, box_code_size), dtype=all_anchors.dtype)
    bbox_targets_1 = np.zeros((num_inside, box_code_size), dtype=all_anchors.dtype)
    bbox_targets_2 = np.zeros((num_inside, box_code_size), dtype=all_anchors.dtype)
    if len(gt_boxes) > 0:
        # bbox_targets[fg_inds, :] = box_encoding_fn(
        #     anchors[fg_inds, :], gt_boxes[anchor_to_gt_argmax[fg_inds], :])
        bbox_targets[fg_inds, :] = box_encoding_fn(
        ...
        bbox_targets_1[fg_inds, :] = box_encoding_fn(
            gt_boxes_1[anchor_to_gt_argmax[fg_inds], :], anchors[fg_inds, :]
        )
        bbox_targets_2[fg_inds, :] = box_encoding_fn(
            gt_boxes_2[anchor_to_gt_argmax[fg_inds], :], anchors[fg_inds, :]
        )
    # Bbox regression loss has the form:
    #   loss(x) = weight_outside * L(weight_inside * x)
    # Inside weights allow us to set zero loss on an element-wise basis
    ...
    ret = {
        "labels": labels,
        "bbox_targets": bbox_targets,
        "bbox_targets_1": bbox_targets_1,
        "bbox_targets_2": bbox_targets_2,
        "bbox_outside_weights": bbox_outside_weights,
        "assigned_anchors_overlap": fg_max_overlap,
        "positive_gt_id": gt_pos_ids,
    }
