...
class RPN_LVX(nn.Module):
        ...
        **kwargs
        ...
        for i, layer_num in enumerate(self._layer_nums):
            ...
            if i - self._upsample_start_idx >= 0:
                ...
                fusions_1.append(fusion_1)
                fusions_add_1.append(fusion_add_1)
                # fusion_2 = Sequential(
                #     nn.Conv2d(
                #         2*self._num_upsample_filters[i-self._upsample_start_idx],
                #         self._num_upsample_filters[i-self._upsample_start_idx],
                #         kernel_size=3,
                #         padding=1,
                #         bias = False, 
                ...
                #         nn.ReLU(),
                # )
                # fusions_2.append(fusion_2)
        self.blocks = nn.ModuleList(blocks)
        self.deblocks = nn.ModuleList(deblocks)
        self.fusions_1 = nn.ModuleList(fusions_1)
        self.fusions_add_1 = nn.ModuleList(fusions_add_1)
    ...
    def forward(self, x,x_1,x_2):
        ...
        for i in range(len(self.blocks)):
            x = self.blocks[i](x)
            x_1 = self.blocks[i](x_1)
            x_2 = self.blocks[i](x_2)
            if i - self._upsample_start_idx >= 0:
                ups.append(self.deblocks[i - self._upsample_start_idx](x))
                ups_1.append(self.deblocks[i - self._upsample_start_idx](x_1))
                ups_2.append(self.deblocks[i - self._upsample_start_idx](x_2))
